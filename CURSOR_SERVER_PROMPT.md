...
```
Обновить код бэкенда RYTMOX на сервере.
...
4. Проверить логи pm2 на ошибки (pm2 logs rytmox-backend --lines 20)
5. Убедиться, что переменная окружения OPENAI_API_KEY установлена

ВАЖНО: Миграция для ingredients уже применена вручную через Supabase Dashboard.
Код обновлен для работы с JSONB вместо TEXT для поля ingredients.

Покажи результаты всех команд и убедись, что сервер запущен без ошибок.
```
