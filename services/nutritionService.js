...
ВАЖНО: Если описание не является описанием еды или блюда, верни ошибку в JSON с полем "error": "not_food". НЕ возвращай "название блюда" или "неизвестное блюдо" - это недопустимо.
...
КРИТИЧЕСКИ ВАЖНО:
1. Всегда учитывай размер порции (большая = ×2, маленькая = ×0.5)
2. Приоритет весу и количеству над описанием размера
3. Учитывай способ приготовления (жареное = +20-30% калорий)
4. Учитывай все добавки (масло, сахар, соусы)
5. Исправляй опечатки в названиях
6. Валидируй разумность значений
...
Возвращай ТОЛЬКО валидный JSON без дополнительного текста.`,
    ...
    // Проверка на ошибку "не еда"
    if (parsed.error === "not_food" || parsed.error) {
      return {
        data: null,
        error: {
